...
$95.5\%$ internally connected. We then specify a falsifiable protocol --- not yet
executed --- to test whether causal regional memory improves task success and
reduces hallucination relative to classical RAG, GraphRAG, MemGPT and LangGraph
on long-horizon repository tasks. We are explicit throughout about what is
\emph{proven} (formal structure, determinism, locality) versus \emph{hypothesised}
(downstream agent gains), so that the contribution can be evaluated honestly.
\end{abstract}
...
\section{Proposed comparative study (not yet run)}\label{sec:protocol}

The thesis worth testing is: \emph{does causal regional memory help an agent solve
long-horizon tasks better than chunk-based retrieval?} We cannot answer this with
the implementation alone --- it requires LLM rollouts, baselines and datasets we
have not run --- so we specify a falsifiable protocol and \textbf{report no
numbers for it}.
